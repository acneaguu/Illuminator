...
def plot_load_profile(load_df, day_of_year, number_houses):
    ...
    colors = ['brown', 'black', 'yellow', 'darkgreen', 'tomato', 'blue', 'grey', 'limegreen', 'skyblue', 'bisque']

    #plt.plot(load_doy.index, load_doy['load'])
    plt.plot(load_doy.index, load_doy['load'], color='black', label='Total Load', linewidth=2)
